...
class TestModelUser(TestCase):
    ...
    def setUpTestData(cls):
       User.objects.create(full_name = 'Иванов Иван Иванович', birth_day = datetime.date(1997, 5, 2), phone = '+375(29)508-80-02')

    def test_full_name_label(self):
    	user = User.objects.get(id = 1)
    	field_label = user._meta.get_field('full_name').verbose_name # Получение метаданных поля для получения необходимых значений
    	self.assertEquals(field_label, 'full name')# Сравнить значение с ожидаемым результатом
    ...
    def test_str_is_full_name(self):
    	user = User.objects.get(id = 1)
    	expected_object_name = '%s' % (user.full_name)
...
class TestModelBook(TestCase):
    ...
    def setUpTestData(cls):
    	...
    	Book.objects.create(reader = user, book_name = 'Чистый код', author_full_name = 'Роберт Мартин', year = 2020)
